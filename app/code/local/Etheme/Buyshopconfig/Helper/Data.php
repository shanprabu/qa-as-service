...
class Etheme_Buyshopconfig_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getProductHtml($_product, $el, $widthBig, $heightBig, $span = 2, $special = false, $list_mode = false, $disable_info = false, $grid = true,$price,$izotope=false,$category_id=0)
    {
        $output = '';
        $description = '';
        ...
        $output .= '<div class="span' . $span . ' '.$category_id.' product">
                        <div class="product-image-wrapper ' . $is_hover . '  '.(($grid)?'animate scale':'').'">
    ...
    public function outputDiscountLabel($_product)
    {
        ...
        if (!($_product->type_id == 'grouped' || $_product->type_id == 'bundle')) {
            ...
            if($discount!=0) return '<div class="sale_discount img-rounded">'.$discount.'%</div>'; else return '';
        } else return '';
    ...
    function refreshStoreCss($store)
    {
        ...
        $path = Mage::getBaseDir() . '/' . 'skin/frontend/buyshop/default/css/colors/';
